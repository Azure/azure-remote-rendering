...
#ifdef USE_REMOTE_RENDERING
            ID3D11Buffer* const cb[] = {m_viewProjectionCBuffer.get()};
            m_deviceContext->VSSetConstantBuffers(1, (UINT)std::size(cb), cb);
            program->RenderARR(m_deviceContext.get());
#endif
