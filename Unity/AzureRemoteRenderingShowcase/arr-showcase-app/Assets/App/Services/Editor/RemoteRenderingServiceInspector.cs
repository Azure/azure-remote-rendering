...
{
    [MixedRealityServiceInspector(typeof(IRemoteRenderingService))]
    public class RemoteRenderingServiceInspector : BaseMixedRealityServiceInspector
    {
        ...
        private void DrawSessionControls(IRemoteRenderingService service)
        {
            ...
            if (GUILayout.Button("Inspector") && service.PrimaryMachine?.Session.Status == Azure.RemoteRendering.RenderingSessionStatus.Ready)
            {
        ...
        private static async Task ApplySettings(IRemoteRenderingService service)
        {
            ...
            {
                Debug.LogError($"Failed to reload profile settings. Exception: {ex.ToString()}");
            }
        ...
        private static async void Connect(IRemoteRenderingService service)
        {
            ...
            {
                Debug.LogError($"Failed to obtain machine for connection. Exception: {ex.ToString()}");
            }
            ...
            if (machine != null)
            {
                ...
                {
                    Debug.LogError($"Failed to connect to machine. Exception: {ex.ToString()}");
                }
            }
        }
        ...
        private static async void Disconnect(IRemoteRenderingService service)
        {
            ...
            if (machine != null)
            {
                ...
                {
                    Debug.LogError($"Failed to disconnect from machine. Exception: {ex.ToString()}");
