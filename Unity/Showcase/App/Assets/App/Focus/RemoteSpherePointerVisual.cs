...
public class RemoteSpherePointerVisual : MonoBehaviour
{
    ...
    private void CheckInitialization()
    {
        ...
        {
            Debug.LogFormat(LogType.Error, LogOption.NoStacktrace, null, "{0}",  $"No near pointer found on {gameObject.name}.");
        }
    ...
    private void CheckAsset(object asset, string fieldname)
    {
        if (asset == null)
        {
            Debug.LogFormat(LogType.Error, LogOption.NoStacktrace, null, "{0}",  $"No {fieldname} specified on {gameObject.name}.SpherePointerVisual. Did you forget to set the {fieldname}?");
        }
    }
