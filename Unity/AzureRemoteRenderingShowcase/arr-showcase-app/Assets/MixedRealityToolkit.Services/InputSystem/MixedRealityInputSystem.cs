...
namespace Microsoft.MixedReality.Toolkit.Input
{
    ...
    public class MixedRealityInputSystem : BaseDataProviderAccessCoreSystem, IMixedRealityInputSystem, IMixedRealityCapabilityCheck
    {
        ...
        public void RaisePointerDown(IMixedRealityPointer pointer, MixedRealityInputAction inputAction, Handedness handedness = Handedness.None, IMixedRealityInputSource inputSource = null)
        {
            ...
            GameObject target = pointer.Result?.Details.Object;

            // Unity overrides the == operator, so 'target' may not really be null.
            // Unity doesn't (and can't) override the Null-conditional operator (?),
            // so we can't also rely on this operator. If Unity claims this is null,
            // its because the native object backing the GameObject has been destroyed
            // via an Object.Destroy(...) call.
            Transform currentObject = target == null ? null : target.transform;
