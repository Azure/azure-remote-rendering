...
public class CustomBuilder
{
    ...
    private async void BuildAppx(CustomBuildInfo customAppxBuildInfo)
    {
        ...
        {
            appxSuccess = false;
            Debug.LogErrorFormat("Exception occurred when creating AppX package. {0}", ex.ToString());
        }
    ...
    private static void SetLockReloadAssemblies(bool value)
    {
        ...
        {
            Debug.LogErrorFormat("Exception occurred when trying to lock assemblies for building. {0}", ex.ToString());
        }
    }
