...
{
    /// <summary>
    /// Custom gaze provider to add a <see cref="RemotePointerCaster"/> to the gaze pointer,
    /// so it can handle Azure Remote Rendering content.
    /// </summary>
    /// <remarks>
    /// The original <see cref="GazeProvider"/> and more importantly the <see cref="GazeProvider.InternalGazePointer"/>
    /// is very locked down and doesn't allow for extending or overriding the gaze pointer.
    /// For this reason, we have to use a slightly different approach here to provide a remote capable gaze pointer.
    ///
    /// So the process is as follows:
    ///   1. The original <see cref="MixedReality.Toolkit.Input.GazeProvider"/> from the MRTK was copy & pasted below in this file.
    ///   2. Change <see cref="GazeProvider.gazePointer"/> access modifier to "protected".
    ///   3. Change <see cref="GazeProvider.InternalGazePointer"/> from "private" to "protected".
    ///   4. Implement new <see cref="RemoteGazeProvider"/> and <see cref="RemoteGazePointer"/> to also cast against remote content.
    /// </remarks>
    public class RemoteGazeProvider : GazeProvider
    {
        private RemoteGazePointer remoteGazePointer;

        protected override void OnEnable()
        {
            remoteGazePointer?.OnEnable();
            base.OnEnable();
        }

        protected override void OnDisable()
        {
            remoteGazePointer?.OnDisable();
            base.OnDisable();
        }

        private static readonly ProfilerMarker InitializeGazePointerPerfMarker = new ProfilerMarker("[Showcase] RemoteGazeProvider.InitializeGazePointer");
        ...
        internal override IMixedRealityPointer InitializeGazePointer()
        {
            using (InitializeGazePointerPerfMarker.Auto())
            {
                ...
                // Save an upcast reference for the OnEnable/OnDisable calls.
                remoteGazePointer = new RemoteGazePointer(this, "Remote Gaze Pointer", null, raycastLayerMasks, maxGazeCollisionDistance, gazeTransform, stabilizer);
                gazePointer = remoteGazePointer;
        ...
        /// <summary>
        /// Custom gaze pointer to also cast against remote rendered content.
        /// </summary>
        private class RemoteGazePointer : InternalGazePointer, IRemotePointer, IMixedRealityQueryablePointer
        {
            ...
            public RemoteGazePointer(GazeProvider gazeProvider, string pointerName, IMixedRealityInputSource inputSourceParent, LayerMask[] raycastLayerMasks, float pointerExtent, Transform gazeTransform, BaseRayStabilizer stabilizer)
                : base(gazeProvider, pointerName, inputSourceParent, raycastLayerMasks, pointerExtent, gazeTransform, stabilizer)
            {
            }
            ...
            private static readonly ProfilerMarker OnPreSceneQueryPerfMarker = new ProfilerMarker("[Showcase] RemoteGazePointer.OnPreSceneQuery");
            ...
            private static readonly ProfilerMarker OnSceneQueryPerfMarker = new ProfilerMarker("[Showcase] RemoteGazePointer.OnSceneQuery");
            ...
            public bool OnSceneQuery(LayerMask[] prioritizedLayerMasks, bool focusIndividualCompoundCollider, out MixedRealityRaycastHit hitInfo, out RayStep ray, out int rayStepIndex)
            {
                ...
                {
                    bool localResult = OnLocalSceneQuery(prioritizedLayerMasks, focusIndividualCompoundCollider, out hitInfo, out ray, out rayStepIndex);

                    // When focus locked, this pointers 'OnSceneQuery' is still invoked by the GGV Pointer and 'FocusProvider.UpdateGazeProvider'.
                    // We must not query remotePointerCaster result, otherwise it will loose the remote focus entity and drag won't properly work.
                    if (IsFocusLocked)
                    {
                        return localResult;
                    }
            ...
            {
                bool querySuccessful = OnSceneQuery(prioritizedLayerMasks, focusIndividualCompoundCollider, out MixedRealityRaycastHit hitInfo, out _, out _);
            ...
            /// <summary>
            /// Implementation copied from <see cref="MixedReality.Toolkit.Input.BaseControllerPointer.OnSceneQuery"/>.
            /// </summary>
            private bool OnLocalSceneQuery(LayerMask[] prioritizedLayerMasks, bool focusIndividualCompoundCollider, out MixedRealityRaycastHit hitInfo, out RayStep Ray, out int rayStepIndex)
            {
                ...
                switch (SceneQueryType)
                {
                    ...
                    case SceneQueryType.SphereCast:
                        ...
                        break;
                    default:
                        throw new Exception("The Remote Gaze Pointer does not handle non-raycast scene queries");
                }
        ...
        IMixedRealityInputHandler
    {
        ...
        protected InternalGazePointer gazePointer = null;
        ...
        protected class InternalGazePointer : GenericPointer
        {
