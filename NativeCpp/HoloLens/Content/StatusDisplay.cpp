...
StatusDisplay::StatusDisplay(const std::shared_ptr<DX::DeviceResources>& deviceResources)
...
void StatusDisplay::Render()
{
    ...
    if (!m_loadingComplete || (!m_textEnabled && !m_imageEnabled))
    {
        return;
    }
...
void StatusDisplay::CreateDeviceDependentResources()
{
    ...
    task<void> createPSTask([this]() {
        winrt::check_hresult(
            m_deviceResources->GetD3DDevice()->CreatePixelShader(PixelShader_txt, sizeof(PixelShader_txt), nullptr, m_pixelShader.put()));
    ...
    if (!m_usingVprtShaders)
    {
        ...
        createGSTask = task<void>([this]() {
            winrt::check_hresult(m_deviceResources->GetD3DDevice()->CreateGeometryShader(
                GeometryShader_txt, sizeof(GeometryShader_txt), nullptr, m_geometryShader.put()));
        });
    }
    ...
    task<void> createQuadTask = shaderTaskGroup.then([this]() {
        ...
        static const VertexPositionTexCoord quadVertices[] = {
